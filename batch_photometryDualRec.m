function batch_photometryDualRec
...
sessions = dir('*mdm*1905*'); % be at the proper folder if you want to do this
currentDir = pwd;
...
for i=1:size(sessions,1)
    ...
    if sum(contains(matFiles,strcat(sessions(i).name(1:sN2(1)-1), sessions(i).name(sN1(1):sN1(2)-1))))==0
        % tests if file is already there (only for single recs 28-5-2019)
        
        % Check if it's a dual recording or a single recording
        photometry_completeDualRec(path_to_data)
