...
function sessionData = takeTraces8(rawData, sessionData)
...
for rsp = 1:2
    for trial = 1:size(sessionData.trialstart,1)
        ...
        % Clear next trial space from traces
        ...
        elseif trcEnd>(numel(sessionData.adjCaSig))
            finalMedTraces(trial,:) = [nan(abs(nanWindowS),1); sessionData.adjCaSig(trcStart:numel(sessionData.adjCaSig))...
